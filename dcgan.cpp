//
// Created by kwoodle on 1/9/20.
// Sample from Pytorch C++ Frontend libtorch
...
//const char* kDataFolder = "/mnt/EVOLinux/kws_space/Fedora/data";
...
// How many workers to load datasets
...
int main(int argc, char* argv[]) {
    ...
    if (torch::cuda::is_available()) {
        ...
        std::cout << "No CUDA. Exiting" << std::endl;
        return 1;
    }
    ...
    auto data_loader = torch::data::make_data_loader(
            std::move(dataset),
            torch::data::DataLoaderOptions().batch_size(kBatchSize).workers(kWorkers));
    ...
    std::cout << "Training complete! using data folder " << kDataFolder << std::endl;
    logger.log("Training complete! using data folder " + string(kDataFolder));
    std::cout << "Duration: " << duration_cast<milliseconds>(d).count()/60000. << " minutes for " << kNumberOfEpochs << " Epochs."
              << std::endl;
    logger << logger.tstamp() << " Duration: " << duration_cast<milliseconds>(d).count()/60000. << " minutes for " << kNumberOfEpochs << " Epochs."
              << std::endl;
}
